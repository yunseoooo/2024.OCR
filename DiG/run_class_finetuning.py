...
# from models.model_builder_with_cirm import RecModel_with_CIRM
...
def main(args, ds_init):
    ...
    # local_rank = int(os.environ['LOCAL_RANK'])      ##### 추가 
    # torch.cuda.set_device(local_rank)               ##### 추가
    device = torch.device(args.device)   # 원본
    # device = torch.device(f'cuda:{local_rank}') # 변경
        ...
        if args.output_dir and args.save_ckpt:
            ...
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                utils.save_model(
                    ...
                    loss_scaler=loss_scaler, epoch=epoch, real_epoch=epoch, model_ema=model_ema)
        if data_loader_val is not None and ((epoch + 1) % 5 == 0):
            ...
            if max_accuracy < test_stats["acc"]:
                ...
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="best", real_epoch=epoch, model_ema=model_ema)
