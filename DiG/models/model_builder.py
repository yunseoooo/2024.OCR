...
class ContentInformationExtractor(nn.Module):
    def __init__(self, channels_in, channels_out):
        super().__init__()
        self.conv = nn.Conv2d(channels_in, channels_out, kernel_size=1)  # Simplicity as per paper
    
    def forward(self, x):
        return self.conv(x)

class OrientationInformationExtractor(nn.Module):
    def __init__(self, channels_in, channels_out):
        super().__init__()
        self.conv = nn.Conv2d(channels_in, channels_out, kernel_size=1)
        self.pool = nn.AdaptiveAvgPool2d(1)  # Global pooling as per paper
...
class CharacterReconstructor(nn.Module):
    def __init__(self, content_channels, orientation_channels):
        super().__init__()
        # Assuming concatenation of content and orientation features
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(content_channels + orientation_channels, 512, kernel_size=5, stride=2, padding=2, output_padding=1),
            nn.ReLU(),
            # Additional deconv layers as per the paper's architecture
        )

    def forward(self, content, orientation):
        combined = torch.cat([content, orientation.expand_as(content)], dim=1)  # Ensure orientation is broadcasted correctly
...
class RecModel(nn.Module):
  ...
  def forward(self, x, aug_x, contrast_label, is_training, cal_covstat, apply_wt):
    x, tgt, tgt_lens = x
    if self.insert_sem:
      enc_x, rec_score = self.encoder((x, self.trg_word_emb))
    else:
      enc_x, shade_output = self.encoder(x, aug_x=aug_x, contrast_label=contrast_label, is_training=is_training, cal_covstat=cal_covstat, apply_wt=apply_wt)
...
class RecModel_with_CIRM(nn.Module):
  def __init__(self, args):
    super(RecModel, self).__init__()
