...
def train_class_batch(model, samples, target, tgt_lens, criterion, criterion_aux=None, args=None, teacher_model=None, metric_logger=None):
    ...
    if isinstance(outputs, tuple):
        if teacher_model is not None:
            ...
            # outputs, sem_feat, sem_feat_attn_maps, dec_attn_maps, reconstructed_images = outputs
                ...
                if max_accuracy < test_stats["acc"]:
                    ...
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            ...
                            loss_scaler=loss_scaler, epoch="best", real_epoch=epoch, model_ema=model_ema)  # model_without_ddt=model.module << 원본

        if (step >= 1 and step % (args.eval_freq * 10) == 0) and ((epoch + 1) % 10 == 0):
            utils.save_model(
                args=args, model=model, model_without_ddp=model, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch="{0}_{1}".format(epoch, step), real_epoch=epoch, model_ema=model_ema) # model_without_ddt=model.module << 원본
