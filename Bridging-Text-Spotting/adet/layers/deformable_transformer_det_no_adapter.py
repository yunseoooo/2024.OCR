...
class DeformableTransformer_Det(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        spatial_shapes = []
        # contoal_feat = self.fpn_adapter(srcs[0], srcs[1], srcs[2], srcs[3])
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            bs, c, h, w = src.shape
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)
            # src = contoal_feat[lvl] + src
        ...
        for i in range(self.num_feature_levels):
            if i < self.num_feature_levels - 1:
                ...
                split_size_or_sections[i] = output_memory.shape[1] - level_start_index[i]
        y = torch.split(output_memory, split_size_or_sections, dim=1)

        encoder_feat = []
        num_cur_levels = 0
        for i, z in enumerate(y):
            encoder_feat.append(z.transpose(1, 2).view(bs, -1, spatial_shapes[i][0], spatial_shapes[i][1]))

        return hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, encoder_feat
