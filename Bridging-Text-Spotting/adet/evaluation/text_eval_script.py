                        ...
                        if gtRectMat[gtNum] == 0 and detRectMat[detNum] == 0 and gtNum not in gtDontCarePolsNum and detNum not in detDontCarePolsNum :
                            if iouMat[gtNum,detNum]>evaluationParams['IOU_CONSTRAINT']:
                                ...
                                if evaluationParams['WORD_SPOTTING']:
                                    ...
                                    try:
                                        if CTW:
                                            correct = transcription_match(gtTrans[gtNum].replace(' ','').upper(),detTrans[detNum].replace(' ','').upper(),evaluationParams['SPECIAL_CHARACTERS'],evaluationParams['ONLY_REMOVE_FIRST_LAST_CHARACTER'])==True
                                        else:
...
def text_eval_main(det_file, gt_file, is_word_spotting, ctw):
    global WORD_SPOTTING
