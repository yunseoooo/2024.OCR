...
class TextEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if not self.use_customer_dictionary:
            self.CTLABELS = list(string.printable[:-6])
        else:
        ...
        if "rotate" in dataset_name:
            if "mpsc" in dataset_name:
                self._text_eval_gt_path = "datasets/evaluation/gt_mpsc_rotate.zip"
    ...
    def evaluate_with_official_code(self, result_path, gt_path):
        if "ctw1500" in self.dataset_name:
            return text_eval_script.text_eval_main(det_file=result_path, gt_file=gt_path, is_word_spotting=self._word_spotting, ctw=True)
        else:
            return text_eval_script.text_eval_main(det_file=result_path, gt_file=gt_path, is_word_spotting=self._word_spotting, ctw=False)
