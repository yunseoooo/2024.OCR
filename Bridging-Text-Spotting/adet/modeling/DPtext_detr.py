...
# from .bridge_deformable import Bridge
...
class TransformerPureDetector(nn.Module):
    def __init__(self, cfg):
        ...
        # checkpoint = torch.load("DiG/checkpoint/mpsc/0716/checkpoint-best.pth", map_location='cpu')
        # checkpoint = torch.load("DiG/checkpoint/final_checkpoint-9.pth")
        checkpoint = torch.load("DiG/checkpoint/mpsc/real_lmdb_128/checkpoint-best.pth")
        # checkpoint = torch.load("DiG/checkpoint/mpsc/real_lmdb_128/checkpoint-9.pth")
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            rec_img = self.box_pooler([tmp_images.tensor], target_boxes)  # RoIPooler
            rec_feat = self.box_pooler2(encoder_feat[:-1], target_boxes)  # RoIPooler
            ...
            #     x_min, y_min, x_max, y_max = box.tensor
            #     width = x_max - x_min
            #     height = y_max - y_min
            
            #     # Check if height is more than twice the width
            #     if height > 2 * width:
            #         breakpoint()  # Set breakpoint here if the condition is met

            rec_img = self.box_pooler([tmp_images.tensor], boxes_results)   # 32 x 128
            # from torchvision.transforms.functional import to_pil_image
            # pil_data = to_pil_image(rec_img[0])
            # pil_data.save("rec_img.jpg")
            # breakpoint()
            rec_feat = self.box_pooler2(encoder_feat[:-1], boxes_results)   # 32 x 128
